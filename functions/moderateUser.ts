...
Deno.serve(async (req) => {
    try {
        ...
        // Check permissions
        const allowedRoles = ['moderator', 'admin', 'superuser'];
        if (!moderator || !allowedRoles.includes(moderator.user_role)) {
        ...
        if (roleHierarchy[targetPlayer.user_role] >= roleHierarchy[moderator.user_role]) {
            return Response.json({ 
                success: false, 
                error: 'Cannot moderate users with equal or higher role' 
            }, { status: 403 });
        }
        ...
        switch (action_type) {
            case 'spam_penalty':
                ...
                // Limit penalty based on role
                if (moderator.user_role === 'moderator' && penalty > 10) {
                    penalty = 10;
                } else if (moderator.user_role === 'admin' && penalty > 20) {
                    penalty = 20;
                }
            ...
            case 'mute':
                ...
                // Limit duration based on role
                if (moderator.user_role === 'moderator' && muteDuration > 24) {
                    muteDuration = 24;
                } else if (moderator.user_role === 'admin' && muteDuration > 168) {
                    muteDuration = 168;
                }
